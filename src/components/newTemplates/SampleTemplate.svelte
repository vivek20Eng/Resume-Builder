<script>
	...
	let projects = [];
	let workExperience = [];
	// Helper function to convert proficiency levels into star rating
	function getStarRating(proficiency) {
		const maxStars = 3;
		const filledStars =
...
<section class="m-5 border rounded-lg p-5 bg-gray-100">
		...
		{#each languages as { language, proficiency }, index (index)}
			<li class="mb-1">
				...
				{#if proficiency === 'beginner'}
					{#each Array(getStarRating(proficiency).filledStars) as _, i}
						<i class="fas fa-star text-yellow-400"></i>
					{/each}
					{#each Array(getStarRating(proficiency).emptyStars) as _, i}
	...
	<hr class="mb-5" />
	<ul class="text-sm">
		{#each Object.values(socialMedia) as { id, platform, url }, index (index)}
			<li class="mb-2">
				<span class="font-semibold text-gray-800">
					<i class="fab fa-{platform} mr-2"></i>
					{platform}
				</span><br />
				<a href={url} target="_blank" rel="noopener noreferrer" class="text-blue-600">{url}</a>
			</li>
		{/each}
	</ul>
	<hr class="my-5" />
